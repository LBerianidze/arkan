...
// settings
$row = $db->getAllRecords(DB_TBLPREFIX . '_configs', '*', ' AND cfgid = "' . 1/*$didId*/ . '"');
...
// Get member details
    ...
    $plan = $db->getAllRecords('ar_mbrplans', '*', 'and `idmbr`=' . $mbrstr['id'], '', 'limit 1')[0];
} catch (Exception $ex) {

}
$member_plan_id = $plan ? $plan['mppid'] : 1;
...
$subscribes = $db->getAllRecords(DB_TBLPREFIX . '_subscribeplans', '*', ' AND ppid = "' . $member_plan_id/*$didId*/ . '"');
// baseplan
...
// paymentgate
$row = $db->getAllRecords(DB_TBLPREFIX . '_paygates', '*', ' AND paygid = "' . $member_plan_id /*$didId*/ . '"');
foreach ($row as $value) {
