...
function badgembrplanstatus($statusid, $mpstatus = 0)
{
    ...
    switch ($statusid) {
        case "1":
            $statusbadge .= "<span class='badge badge-success'>Активный</span>";
            break;
        case "2":
        ...
        case "3":
            ...
            break;
        default:
            $statusbadge .= "<span class='badge badge-light'>Неактивный</span>";
    }
    switch ($mpstatus) {
        case "0":
...
function formatdate($datetimestr, $type = 'd')
{
    ...
    $en_weekdays = array( 'Mon', 'Tue', 'Wed', 'Thur', 'Fri', 'Sat', 'Sun' );
    $ru_months = array( 'Янв.', 'Фев.', 'Мар.', 'Апр.', 'Май.', 'Июн.', 'Июл.', 'Авг.', 'Сен.', 'Окт.', 'Ноя.', 'Дек.' );
    $en_months = array( 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec' );
    $en_date = str_replace( $en_months,$ru_months, date($dtformat, strtotime($datetimestr)) );
    $en_date = str_replace( $en_weekdays,$ru_weekdays, $en_date );
...
function passmeter($password)
{
    ...
    if (!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
        return 'Пароль должен содержать минимум одну букву в верхнем и нижнем регистре, одну цифру, один спец символ и быть длиной не менее 8 символов';
    } else {
        return 1;
    }
}
...
function updatePlanRow($member_plan_id)
{
    global $db, $cfgrow, $bpprow;
    $bpprow = array();
    $row = $db->getAllRecords(DB_TBLPREFIX . '_baseplan', '*', ' AND bpid = "' . $member_plan_id/*$didId*/ . '"');
    foreach ($row as $value) {
        $bpprow = array_merge($bpprow, $value);
    }

// payplan
    $row = $db->getAllRecords(DB_TBLPREFIX . '_payplans', '*', ' AND ppid = "' . $member_plan_id /*$didId*/ . '"');
...
function regmbrplans($mbrstr = array(), $refmpid = 0, $ppid = 1)
{
    ...
    if ($bpprow['planstatus'] == 1 && count($sql) < 1) {
        ...
        if ($insert) {
            $_SESSION['dotoaster'] = "toastr.success('Регистрация прошла успешно!', 'Успех');";
