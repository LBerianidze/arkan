...
class Database
{
     ...
     * @return int    number of affected rows
     */
    public function updateAll($tableName, array $set)
    {
        $arrSet = array_map(
            function ($value) {
                return $value . '=:' . $value;
...
function dosupdate($lickey, $myver, $tover)
{
    ...
    echo '.';
    $arrdata = array('lickey' => $lickey, 'myver' => $myver64, 'tover' => $tover64, 'do' => 'dbupdate');
...
function checknewver()
{
    ...
    if ($lastcheckdate <= $nextcheckdate) {
        $dataArr = array('lickey' => $cfgrow['lickey'], 'myver' => $cfgrow['softversion'], 'do' => 'ver', 'ccid' => $ccid, 'lictype' => $lictype, 'licpk' => $licpk);
        $initurl = "https://www.mlmscript.net/~enverifykey/api.php";
