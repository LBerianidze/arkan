...
class Paginator {
    ...
    function paginate() {
        ...
        if ($this->num_pages > 1) {
            ...
            $this->return .= ($_GET['page'] == 'All') ? "<li class='page-item active'><a class=\"page-link\" hidden href=\"javascript:;\">Все</a></li>" : "<li class='page-item'><a class=\"page-link\" hidden href=\"$_SERVER[PHP_SELF]?page=1&ipp=All$this->querystring\">Все</a></li></ul>";
        }
        else {
            for ($i = 1; $i <= $this->num_pages; $i++) {
    ...
    function display_items_per_page() {
        ...
        return "<span class=\"text-muted\">Показать:</span> <select class=\"border form-control-sm text-muted\" onchange=\"window.location='$_SERVER[PHP_SELF]?page=1&ipp='+this[this.selectedIndex].value+'$this->querystring';return false\">$items</select>";
    }
    ...
    function display_jump_menu() {
        ...
        return "<span class=\"text-muted\">Страница:</span> <select class=\"border form-control-sm text-muted\" onchange=\"window.location='$_SERVER[PHP_SELF]?page='+this[this.selectedIndex].value+'&ipp=$this->items_per_page$this->querystring';return false\">$option</select>";
    }

    function items_total() {
        return "Всего: " . $this->items_total;
    }
