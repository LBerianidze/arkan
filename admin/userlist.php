...
$userData = $db->getRecFrmQry("SELECT * FROM " . DB_TBLPREFIX . "_mbrs AS mbr LEFT JOIN " . DB_TBLPREFIX . "_mbrplans AS plan ON id = idmbr LEFT JOIN " . DB_TBLPREFIX . "_sessions AS ses ON mbr.id = ses.sesidmbr WHERE 1 " . $condition . ' GROUP BY id ' . $sqlshort . $pages->limit . "");
?>
                ...
                if (count($userData) > 0) {
                    ...
                    foreach ($userData as $val) {
                        ...
                        <tr>
                            ...
                            <td data-toggle='tooltip' title='<?php echo myvalidate($val['firstname']) . ' ' . myvalidate($val['lastname']); ?>'><?php echo myvalidate($isuseron . $val['username']); ?></td>
