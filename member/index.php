...
if ($mbrstr['carat_refresh_month'] != date('n'))
{
    $mbrstr['carat_refresh_month'] = date('n');
    $mbrstr['refwallet'] = 0;
    $db->update('ar_mbrs', ['refwallet' => 0, 'carat_refresh_month' => date('n')], array('id' => $mbrstr['id']));
}
...
if ($FORM['dlfn'] && $FORM['dlid'] > 0 && $FORM['l'] == md5($cfgrow['dldir'] . $FORM['dlid'] . date("md")))
{
    ...
    $data = array('fldlcount' => $fldlcount,);
