...
if (isset($FORM['dosubmit']) and $FORM['dosubmit'] == '1')
{
    ...
    if ($txpaytype != '' && $txamount > 0 && $txamount <= $mbrstr['ewallet'])
    {
        ...
        if ($fval > 0)
        {
            ...
            //$txamountfee = ($fcapval <= $txwdrfee && $fcapval >0) ? $fcapval : $txwdrfee;
            $txamountval = $txamount - (($txamount * $fval) / 100);
        }

        // deduct wallet
        $ewallet = $mbrstr['ewallet'] - $txamount;
        $data = array('ewallet' => $ewallet,);
        ...
        $data = array('txdatetm' => $txdatetm, 'txpaytype' => $txpaytype, 'txfromid' => $mbrstr['id'], 'txtoid' => 0, 'txamount' => $txamountval, 'txmemo' => $LANG['g_withdrawstr'], 'txppid' => $mbrstr['mppid'], 'txtoken' => "|WIDR:OUT|, |WDRTXFEE:{$txamountfee}|", 'txstatus' => 0, 'txadminfo' => $txadminfo,);
        ...
        if ($insert)
        {
            ...
            if ($txamountfee > 0)
            {
                ...
                $data = array('txdatetm' => $txdatetm, 'txpaytype' => $txpaytype, 'txfromid' => $mbrstr['id'], 'txtoid' => 0, 'txamount' => $txamountfee, 'txmemo' => $LANG['g_withdrawfee'], 'txppid' => $mbrstr['mppid'], 'txtoken' => "|WDRTXID:{$newtrxid}|, |NOTE:" . base64_encode("WDRID-{$txlogtime}") . "|", 'txstatus' => 1,);
    ...
    <form method="post" action="index.php">
        ...
        <div class="card card-primary">
            <div class="card-header">
                <h4>
                    <?php echo myvalidate($LANG['g_balance']); ?>
                    <span class="<?php echo myvalidate($balanceclor); ?>"><?php echo myvalidate($bpprow['currencysym'] . ' ' . $mbrstr['ewallet']); ?></span>
                    <?php echo myvalidate($bpprow['currencycode']); ?>
                </h4>
            </div>
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <select name='txpaytype' class="custom-select" id="inputGroupSelect05" required="">
                                    ...
                                    {
                                        ?>
                                        <option value="paypalacc">PayPal
                                            (<?php echo myvalidate($mbrpaystr['paypalacc']) ? base64_decode($mbrpaystr['paypalacc']) : '?'; ?>
                                            )
                                        </option>
                                        <?php
                                    }
                                    ...
                                    {
                                        ?>
                                        <option value="coinpaymentsmercid">Bitcoin
                                            (<?php echo myvalidate($mbrpaystr['coinpaymentsmercid']) ? base64_decode($mbrpaystr['coinpaymentsmercid']) : '?'; ?>
                                            )
                                        </option>
                                        <?php
                                    }
                        ...
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <input type="number" id="txamount" name="txamount" min="0" step="0.5"
                                       class="form-control"
                                       onChange="dowithdrawfee('<?php echo myvalidate($fval); ?>', '<?php echo myvalidate($fcapval); ?>', '<?php echo myvalidate($bpprow['currencysym']); ?>');"
                        ...
                        <div class="float-md-right mt-4">
                            <a href="index.php?hal=withdrawreq" class="btn btn-danger"><i class="fa fa-fw fa-redo"></i>Очистить</a>
                            <button type="submit" name="submit" value="withdraw" id="submit"
                                    class="btn btn-primary"<?php echo myvalidate($btnwidrdis); ?>><i
                                        class="fa fa-fw fa-donate"></i> <?= $LANG['m_withdrawreq'] ?></button>
                        </div>
